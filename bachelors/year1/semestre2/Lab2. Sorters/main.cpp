...
void test_sorting(my_type a[], int l, int r, cmp_type cmp, sort_type some_sorting, string name, string complexity){
    ...
    clock_t started_work = clock();
    some_sorting(a, l, r, cmp);
    clock_t stopped_work = clock();
    output_array(a, l, r, fout);
    ...
    info_file << "complexity " << complexity << endl;
    int n = r - l + 1;
    if (complexity.find("N * logN") != -1)
        info_file << (n * log2(n * 1.0)) << endl;
    else if (complexity.find("N ^ 2") != -1)
        info_file << n * n << endl;
    else info_file << n << endl;
    info_file << "Working time " << (double)(stopped_work - started_work) / CLOCKS_PER_SEC << " secs.\n";
    info_file << endl;
}
...
int main()
{
    ...
    test_sorting(A, 0, n - 1, lower, bucket_sort, "bucket_sort", "N + K (worst N ^ 2 * K)");
    test_sorting(A, 0, n - 1, lower, patience_sort, "patience_sort", "N * logN");
    return 0;
}
