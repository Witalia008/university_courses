...
public class Kernel extends Thread {
    ...
    public void step() {
        ...
        if (!wasPageFault) {
            for (i = 0; i < virtPageNum; i++) {
                Page curPage = (Page) memVector.elementAt(i);
                if (curPage.physical != -1) {
                    if (runs % 1 == 0) {
                        curPage.updateCounter();
                    }
                    if (curPage.lastTouchTime > 0) {
                        curPage.R = 0;
                    }
                }
                if (curPage.physical != -1) {
                    curPage.inMemTime = curPage.inMemTime + 10;
                    curPage.lastTouchTime = curPage.lastTouchTime + 10;
                }
            }
            runs++;
            controlPanel.counterLabel.setText(Long.toString(page.counter, 2));
